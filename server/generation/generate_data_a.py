...
# Fonction pour générer des données aléatoires pour la table "historique_générale"
def generate_historique_générale_data(partie_ids, user_ids):
...
# Génération de données aléatoires pour la table "partie_lambda"
parties = []
for _ in range(10):
    partie_data = generate_partie_lambda_data(users)
    query = "INSERT INTO partie_lambda (ID_user, parties_jouees, nombre_vagues_totales, nombre_monstre_total, " \
            "monstres_tues_individuel, temps_total_partie, tours_construites_total, tours_individuel_total, " \
            "tune_totale, score, duree_jeu, win) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    cursor.execute(query, partie_data)
    parties.append(cursor.lastrowid)

connexion.commit()
print("Données générées pour la table 'partie_lambda'")

# Génération de données aléatoires pour la table "historique_générale"
for _ in range(3):
    historique_data = generate_historique_générale_data(parties, users)
    query = "INSERT INTO historique_générale (ID_partie, ID_user) VALUES (%s, %s)"
    cursor.execute(query, historique_data)
...
for _ in range(3):
    statistiques_user_data = generate_statistiques_user_data(users)
    ...
    cursor.execute(query, statistiques_user_data)
